...
public class WikiIndexer {
   ...
   // groups specified with (), entire match in group 0, groups increment from left
   // to right through the pattern
   ...
   // indetify piped links -> [[Albert Einstein|Einstein]] clickable 'Einstein'
   // that takes user to [[Albert Einstein]])
   ...
   /**
    * entry point for building the wiki index. creates a file system dir for the
    * index itelf,
    * sets the language settings, and initializes an index writer.
    *
    * the index writer is important. its the enttity which organizes and builds the
    * index
    * from the corpus
    *
   ...
   public static void buildIndex(Path wikiDir, Path indexDir) throws IOException {
      ...
      // initialize an IndexWriterConfig and instantiate an analyzer and set other
      // settings
   ...
   /**
    * indexes each file that contains thousands of wikipedia pages. Splits the
    * pages contained
   ...
   private static int indexFile(File f, IndexWriter writer) throws IOException {
      ...
      for (int i = 0; i < titles.size(); i++) {
         ...
         if (RE_REDIRECT.matcher(body).find()) {
            continue;
         }
   ...
   /**
    * clean up the body text of a wiki page using the regex patterns specified at
    * the top
    * of this class
    *
